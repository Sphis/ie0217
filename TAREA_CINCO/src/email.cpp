...
void ValidadorEmail::validarNombre(const string& nombre) {
    ...
    if (!regex_match(nombre, nombreRegex)) {
        throw ExcepcionNombre("Debe contener letras, numeros, puntos, guiones o guiones bajos. No debe comenzar ni terminar con un caracter especial, y no debe tener mas de 15 caracteres.");
    }
...
void ValidadorEmail::validarDominio(const string& dominio) {
    ...
    if (!regex_match(dominio, dominioRegex)) {
        throw ExcepcionDominio("Debe contener unicamente letras y puntos. El punto no puede estar al inicio ni al final.");
    }
    ...
    if (dominio.find("..") != string::npos) {
        throw ExcepcionDominio("No debe contener segmentos consecutivos separados por un unico punto.");
    }
}
...
void ValidadorEmail::validarExtension(const string& extension) {
    ...
    if (!regex_match(extension, extensionRegex)) {
        throw ExcepcionExtension("Debe contener entre 2 y 6 letras, ademas, no se permiten numeros ni caracteres especiales, pero puede ser compuesta.");
    }
}